...
public class Program {
    ...
    public static void main(String[] args) {
        ...
        Harvester harvester = new Harvester("A", "B", Color.BLACK, FuelType.Diesel);
        SchoolBus busik = new SchoolBus("MAN", "SB500", Color.YELLOW, FuelType.Diesel);

        RefuelingStation gasprom = new RefuelingStation("ГАЗПром");
        CarWashStation octopus = new CarWashStation("Осьминог");

        RefuelingStation tatneft = new RefuelingStation("ТАТнефть");
        CarWashStation bestwash =new CarWashStation("Лучшая мойка");

        harvester.setRefuelingStation(gasprom);
        harvester.fuel();
        harvester.setCarWahStation(octopus);
        harvester.wipWindshield();
        harvester.wipHeadlights();

        busik.setRefuelingStation(tatneft);
        busik.fuel();
        busik.setCarWahStation(bestwash);
        busik.wipMirrors();
        busik.wipWindshield();
